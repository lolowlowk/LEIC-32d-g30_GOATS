...
{
    table_add_row(dest);
    size_t dest_row_index = dest->row_num - 1;

    for (unsigned short col = 0; col < dest->col_num; col++)
    {
        const char* src_cell = src_row->cells[col];
...
table* table_filter(const table* t, bool (*predicate)(const void *row, const void *context), const void* context)
{
    ...
    // Create result table
    table* result = table_create(t->col_num);
    if (result == NULL)
        return NULL;
